...
try:
    ALLOWED_GROUP_ID = int(
        ALLOWED_GROUP_ID) if ALLOWED_GROUP_ID is not None else None
except ValueError:
    raise ValueError(
        "Environment variable 'ALLOWED_GROUP_ID' must be an integer.")
...
if ADMIN_USERS:
    try:
        ...
        raise ValueError(
            "Environment variable 'ADMIN_USERS' must be a comma-separated list of integers."
        )
else:
    ADMIN_USERS = []
...
def log_bot_activity(update=None, activity=""):
    ...
    if isinstance(update,
                  Update) and update.message and update.message.from_user:
...
async def save_file_metadata(update: Update, file_id: str, chat_id: int,
                             file_name: str, message_id: int, file_type: str,
                             file_size: int) -> None:
    ...
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute("SELECT 1 FROM file_metadata WHERE file_id = ?",
                           (file_id, ))
            if cursor.fetchone():
                await update.message.reply_text(
                    f"File with ID '{file_id}' already exists in the database. Upload discarded."
                )
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (file_id, chat_id, file_name, message_id, file_type,
                  file_size))
            conn.commit()
            log_bot_activity(
                update,
                f"File '{file_name}' uploaded and saved to chat {chat_id}.")
...
async def handle_document(update: Update,
    ...
    if chat_id == ALLOWED_GROUP_ID:
        ...
        await save_file_metadata(update, file_id, chat_id, file_name,
                                 message_id, 'document', file_size)

        # Send confirmation message
        await update.message.reply_text(
            f"Document '{file_name}' uploaded and saved.")
    else:
        await update.message.reply_text(
            "This bot only accepts files from a specific group.")


async def handle_video(update: Update,
                       context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat_id == ALLOWED_GROUP_ID:
        ...
        # Await the save_file_metadata coroutine to handle duplicates and save metadata
        await save_file_metadata(update, file_id, chat_id, file_name,
                                 message_id, 'video', file_size)

        # Send confirmation message
        await update.message.reply_text(
            f"Video '{file_name}' uploaded and saved.")
    else:
        await update.message.reply_text(
            "This bot only accepts videos from a specific group.")


async def search_files(update: Update,
                       context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def show_files(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    keyboard = [[
        InlineKeyboardButton(f"[{human_readable_size(file[5])}] {file[2]}",
    ...
    if start_idx > 0:
        nav_buttons.append(
            InlineKeyboardButton("Back", callback_data="prev_page"))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    welcome_message = (f"Hi, {first_name} {last_name}!\n\n"
                       ...
                       "/chatinfo - Shows chat information\n\n"
                       f"Your User ID: {user_id}")
    await update.message.reply_text(welcome_message)
    log_bot_activity(
        update,
        f"User {first_name} {last_name} (ID: {user_id}) started the bot.")
...
async def feedback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    with open(feedback_file, mode='a', newline='') as file:
        ...
        if not file_exists:
            ...
            writer.writerow([
                'User ID', 'First Name', 'Last Name', 'Feedback', 'Timestamp'
            ])
        writer.writerow([
            user_id, first_name, last_name, feedback_text,
    ...
    if len(feedback_text) > 4096:
        if is_admin(user_id):
            ...
            await update.message.reply_text(
                "The feedback file has been sent to the admin.")
        else:
            await update.message.reply_text(
                "Feedback is too long. Please shorten it or contact the admin directly."
            )


async def sfeedback(update: Update,
                    context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(feedback_content) > 4096:
        await update.message.reply_text(
            "Feedback file is too large to display here. Please check the file directly."
        )
...
async def dfeedback(update: Update,
                    context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async with lock:
        if data == "send_files":
            if chat_id in user_file_queue:
                ...
                await query.answer(
                    text="No files selected. Please select files first.")
        elif data == "next_page":
            ...
            if 0 <= index < len(files):
                ...
                try:
                    await context.bot.copy_message(chat_id=chat_id,
                                                   from_chat_id=file_chat_id,
                                                   message_id=message_id)
                    logger.info(
                        f"Successfully forwarded file ID {file_id} to chat ID {chat_id}."
                    )
                except Exception as e:
                    logger.error(
                        f"Error sending file ID {file_id} to chat ID {chat_id}: {e}"
                    )
            else:
...
async def send_selected_files(update: Update,
    ...
    async with lock:
        if selected_indices:
            for index in selected_indices:
                if 0 <= index < len(files):
                    file = files[index]
                    file_id, file_chat_id, message_id = file[0], file[1], file[
                        3]
                    try:
                        ...
                        await context.bot.send_message(
                            chat_id=chat_id,
                            text=f"Failed to send file: {file_id}")

            # Notify after sending all selected files
            await context.bot.send_message(
                chat_id=chat_id, text="All selected files have been sent.")
        else:
            await context.bot.send_message(
                chat_id=chat_id,
                text="No files were selected or available to send.")

        # Clear the user's queue and update the callback message
        user_file_queue[chat_id] = []
        await update.callback_query.message.edit_text(
            "Files have been sent. Thank you!")
...
async def admin_list_files(update: Update,
                           context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            with get_db_connection() as conn:
                ...
                if context.args and context.args[0].lower() == 'json':
                    ...
                    with open(temp_file_path, 'rb') as file:
                        await update.message.reply_document(
                            document=file, filename='file_metadata.json')
                ...
                if len(file_list) > 4096:  # Telegram message length limit
                    ...
                    with open(temp_file_path, 'rb') as file:
                        await update.message.reply_document(
                            document=file, filename='file_metadata.csv')
...
async def remove_file(update: Update,
                      context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_admin(user_id):
        if context.args:
            ...
            try:
                file_id = int(
                    file_id_str)  # Convert the provided id to an integer
                ...
                with get_db_connection() as conn:
                    ...
                    cursor.execute(
                        "SELECT file_name FROM file_metadata WHERE id = ?",
                        (file_id, ))
                    ...
                    if result:
                        ...
                        await update.message.reply_text(
                            f"File '{file_name}' removed successfully.")
                        log_bot_activity(
                            f"File with id {file_id} removed by admin {user_id}."
                        )
                    else:
                        await update.message.reply_text(
                ...
                await update.message.reply_text(
                    "Invalid id provided. Please provide a numeric id.")
            except sqlite3.Error as e:
                logging.error(f"Error removing file: {e}")
                await update.message.reply_text(f"Error removing file: {e}")
        else:
            await update.message.reply_text(
                "Please provide the id of the file to remove.")
    else:
        await update.message.reply_text(
            "You are not authorized to use this command.")
...
async def show_log(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_admin(user_id):
        ...
        await update.message.reply_text(
            "You are not authorized to view the bot activity log.")


async def clear_log(update: Update,
                    context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_admin(user_id):
        try:
            ...
            log_bot_activity(f"Admin {user_id} cleared the log file."
...
async def chat_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        info_msg = f"<b>📊 Chat Info</b>\n\n"
        info_msg += f"🏷 <b>Title:</b> {getattr(chat, 'title', 'N/A')}\n"
        info_msg += f"🆔 <b>Chat ID:</b> <code>{chat.id}</code>\n"
        info_msg += f"👥 <b>Type:</b> {chat_type}\n"
        info_msg += f"🧑 <b>Username:</b> @{chat.username if chat.username else 'N/A'}\n"
        info_msg += f"🔐 <b>Is Forum:</b> {getattr(chat, 'is_forum', 'N/A')}\n"

        # Description only exists in groups, supergroups, and channels
        description = getattr(chat, 'description', None)
        if description:
            info_msg += f"🧾 <b>Description:</b> {description}\n"

        # Permissions exist only in groups and supergroups
            ...
            if permissions:
                ...
                for field in perm_fields:
                    ...
                    if value is not None:
                        info_msg += f"➤ {field.replace('_', ' ').capitalize()}: {value}\n"

        await update.message.reply_html(info_msg)

    except Exception as e:
        await update.message.reply_text(
            f"⚠️ An error occurred while retrieving chat info:\n<code>{e}</code>",
            parse_mode='HTML')
